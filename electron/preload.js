...
const runtime = deepFreeze({
  ...
  bypassAuth: process.env.SPLICE_BYPASS_AUTH === "1",
  platform: process.platform,
});

contextBridge.exposeInMainWorld("splice", createPreloadApi(runtime));
...
let _spliceAI = { startAudioStream: () => Promise.reject(new Error("spliceAI not loaded")), stopAudioStream: () => Promise.reject(new Error("spliceAI not loaded")), getAudioStatus: () => Promise.reject(new Error("spliceAI not loaded")), onAudioEvent: () => () => {} };
try {
  const { spliceAIBridge } = require("./preload/aiAudio");
  _spliceAI = spliceAIBridge;
} catch (err) {
  if (typeof console !== "undefined" && console && typeof console.error === "function") {
    console.error("[preload] aiAudio bridge load failed", err);
  }
}
contextBridge.exposeInMainWorld("spliceAI", _spliceAI);
